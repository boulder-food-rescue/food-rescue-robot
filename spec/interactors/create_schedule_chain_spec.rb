...
RSpec.describe CreateScheduleChain do
  describe '::call' do
    subject { described_class.call( schedule_chain: chain ) }

    let(:chain) { build(:schedule_chain, :one_time) }
    ...
    it 'does not create rescue logs' do
      expect { subject }.to_not(change { Log.count })
    end
    ...
    context 'schedule is one time and same day' do
      ...
      let(:chain) { create(:schedule_chain, :one_time, detailed_date: Date.current) }
